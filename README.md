...
> - **Au démarrage** : L'initialisation simultanée du conteneur (téléchargement du runtime CUDA, du client F@h, des cores et de la première Work Unit) combinée au test de débit initial de `fah-monitoring.sh` génère un pic de trafic important qui peut déclencher une alerte temporaire.
> - **En routine** : Le test de débit périodique (toutes les 12h) ainsi que le transfert de certaines Work Units ou cores volumineux (plusieurs centaines de Mo) peuvent ponctuellement dépasser les limites.
> 
> *Ajustez les seuils en paramètre du script selon vos besoins et les alertes constatées.*
...
> 💡 **Mind potential false positives**:
> - **At startup**: Container initialization (downloading CUDA runtime, F@h client, cores, and the first Work Unit) combined with `fah-monitoring.sh`'s initial speed test creates a large traffic spike that may trigger a temporary alert.
> - **During routine execution**: The periodic speed test (every 12h) as well as uploading/downloading large Work Units or cores (several hundred MBs) can occasionally exceed the thresholds.
> 
> *Adjust the script parameter thresholds based on your observed alerts and needs.*
