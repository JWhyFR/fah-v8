...
log "Vérification de l'heure système..."

# 1. Heure système locale
LOCAL_TS=$(date -u +%s)
log "Heure système UTC : $LOCAL_TS"
...
if [ -n "$API_TIME" ]; then
  ...
  log "API publique : Impossible de récupérer l'heure."
fi
...
if [[ "$OFFSET" =~ ^[-+]?[0-9.]+$ ]]; then
  ...
  log "Serveur NTP : Impossible de récupérer l'heure."
fi
