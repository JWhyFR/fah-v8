...
# 1. Récupération de la version CUDA supportée par le Driver (ex: 12.2)
...
echo "--- Version cible détectée : $CUDA_MAJOR.$TARGET_MINOR ---"

# 2. Récupération des versions mineures disponibles dans le repo pour cette majeure
# On extrait juste le numéro mineur (ex: "5" pour "cuda-cudart-12-5")
AVAILABLE_MINORS=$(apt-cache pkgnames cuda-cudart-${CUDA_MAJOR}- | grep -oP "${CUDA_MAJOR}-\K\d+" | sort -n)

if [ -z "$AVAILABLE_MINORS" ]; then
    echo "Erreur : Aucun package cuda-cudart-${CUDA_MAJOR}- trouvé."
    exit 1
fi

# 3. Logique de sélection de la version la plus proche
BEST_MINOR=""
MIN_DIFF=999

for m in $AVAILABLE_MINORS; do
    # Calcul de la différence absolue entre la version dispo et la cible
    diff=$(( m - TARGET_MINOR ))
    abs_diff=${diff#-} # Supprime le signe moins si négatif
    ...
    # Si on trouve la version exacte (différence 0), on s'arrête
...
# 4. Installation sécurisée (indispensable sur Vast.ai)
